...
void RawSubscriber::subscribeImpl(ros::NodeHandle& nh, const std::string& base_topic, uint32_t queue_size,
                                  const boost::function<void(const sensor_msgs::ImageConstPtr&)>& callback,
                                  const ros::VoidPtr& tracked_object, const ros::TransportHints& transport_hints)
{
  sub_ = nh.subscribe<sensor_msgs::Image>(base_topic, queue_size, callback, tracked_object, transport_hints);
}
